...
describe('E2E tests', function () {
    ...
    describe('Search Page [ 15 Points ]', async () => {
        ...
        it('Show results with 2 albums for Guest [ 2.5 Points ]', async () => {
            const { get } = await handle(endpoints.search('Melo'));
            get(mockData.catalog.slice(0, 2));
            ...
            await page.fill('[name="search"]', 'Melo');
            await page.click('button >> text="Search"');
            await page.waitForTimeout(interval);
...
async function setupContext(context) {
    ...
    // Profile Page
    await handleContext(context, endpoints.search('Melo'), { get: mockData.catalog.slice(0, 2) });
